...
#ifdef _AIX
extern "C" intptr_t posix_memalign(void **memptr, uintptr_t alignment, uintptr_t size);
#endif
...
inline intptr_t posix_memalign0(void **memptr, uintptr_t alignment, uintptr_t size)
{
...
template<class T>
inline void newAlign(T *&v, uintptr_t len, uintptr_t align)
{
  void *mem=NULL;
  const char *invalid="Invalid alignment requested";
  const char *nomem="Memory limits exceeded";
#ifdef HAVE_POSIX_MEMALIGN
  intptr_t rc=posix_memalign(&mem,align,len*sizeof(T));
#else  
  intptr_t rc=posix_memalign0(&mem,align,len*sizeof(T));
#endif  
  if(rc == EINVAL) Array::ArrayExit(invalid);
  if(rc == ENOMEM) Array::ArrayExit(nomem);
  v=(T *) mem;
  for(uintptr_t i=0; i < len; i++) new(v+i) T;
}

template<class T>
inline void deleteAlign(T *v, uintptr_t len)
{
  for(uintptr_t i=len-1; i > 0; i--) v[i].~T();
  v[0].~T();
#ifdef HAVE_POSIX_MEMALIGN
  free(v);
#else
...
class array1 {
 ...
 public:
  ...
  intptr_t test(intptr_t flag) const {return state & flag;}
  void clear(intptr_t flag) const {state &= ~flag;}
  void set(intptr_t flag) const {state |= flag;}
  void Activate(uintptr_t align=0) {
    if(align) {
  ...
  void CheckActivate(intptr_t dim, uintptr_t align=0) {
    if (test(allocated)) {
      std::ostringstream buf;
  ...
  virtual void Dimension(uintptr_t nx0) {size=nx0;}
  void Dimension(uintptr_t nx0, T *v0) {
  ...
  void CheckActivate(uintptr_t align=0) {
    __checkActivate(1,align);
  }
  
  void Allocate(uintptr_t nx0, uintptr_t align=0) {
    Dimension(nx0);
    CheckActivate(align);
  }
  
  void Reallocate(uintptr_t nx0, uintptr_t align=0) {
    Deallocate();
    Allocate(nx0,align);
  }
  ...
  array1(uintptr_t nx0, uintptr_t align=0) : state(unallocated) {
    Allocate(nx0,align);
  }
  array1(uintptr_t nx0, T *v0) : state(unallocated) {Dimension(nx0,v0);}
  array1(T *v0) : state(unallocated) {Dimension(INTPTR_MAX,v0);}
  ...
  virtual void Check(intptr_t i, intptr_t n, uintptr_t dim, uintptr_t m,
		     intptr_t o=0) const {
    if(i < 0 || i >= n) {
      std::ostringstream buf;
      buf << "Array" << dim << " index ";
  ...
  uintptr_t Nx() const {return size;}
  ...
  T& operator [] (intptr_t ix) const {__check(ix,size,1,1); return v[ix];}
  T& operator () (intptr_t ix) const {__check(ix,size,1,1); return v[ix];}
  T* operator () () const {return v;}
  operator T* () const {return v;}
	
  array1<T> operator + (intptr_t i) const {return array1<T>(size-i,v+i);}
	
  void Load(T a) const {
    __checkSize();
    for(uintptr_t i=0; i < size; i++) v[i]=a;
  }
  void Load(const T *a) const {
    for(uintptr_t i=0; i < size; i++) v[i]=a[i];
  }
  void Store(T *a) const {
    for(uintptr_t i=0; i < size; i++) a[i]=v[i];
  }
  ...
  T Min() {
    ...
    T min=v[0];
    for(uintptr_t i=1; i < size; i++) if(v[i] < min) min=v[i];
    return min;
  }
  T Max() {
    if(size == 0)
    ...
    T max=v[0];
    for(uintptr_t i=1; i < size; i++) if(v[i] > max) max=v[i];
    return max;
  }
  
  std::istream& Input (std::istream &s) const {
    __checkSize();
    for(uintptr_t i=0; i < size; i++) s >> v[i];
    return s;
  }
  ...
  array1<T>& operator /= (T a) {
    __checkSize();
    T ainv=1.0/a;
    for(uintptr_t i=0; i < size; i++) v[i] *= ainv;
    return *this;
  }
  ...
  double L1() const {
    __checkSize();
    double norm=0.0;
    for(uintptr_t i=0; i < size; i++) norm += abs(v[i]);
    return norm;
  }
  ...
  double Abs2() const {
    __checkSize();
    double norm=0.0;
    for(uintptr_t i=0; i < size; i++) norm += abs2(v[i]);
    return norm;
  }
  double L2() const {
    return sqrt(Abs2());
...
class array2 : public array1<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0) {
    nx=nx0; ny=ny0;
    this->size=nx*ny;
  }
  void Dimension(uintptr_t nx0, uintptr_t ny0, T *v0) {
    Dimension(nx0,ny0);
    this->v=v0;
  ...
  void Allocate(uintptr_t nx0, uintptr_t ny0, uintptr_t align=0) {
    Dimension(nx0,ny0);
    __checkActivate(2,align);
  }
	
  array2() : nx(0), ny(0) {}
  array2(uintptr_t nx0, uintptr_t ny0, uintptr_t align=0) {
    Allocate(nx0,ny0,align);
  }
  array2(uintptr_t nx0, uintptr_t ny0, T *v0) {Dimension(nx0,ny0,v0);}
	
  uintptr_t Nx() const {return nx;}
  uintptr_t Ny() const {return ny;}

#ifndef __NOARRAY2OPT
  T *operator [] (intptr_t ix) const {
    return this->v+ix*ny;
  }
#else
  array1<T> operator [] (intptr_t ix) const {
    __check(ix,nx,2,1);
    return array1<T>(ny,this->v+ix*ny);
  }
#endif
  T& operator () (intptr_t ix, intptr_t iy) const {
    __check(ix,nx,2,1);
    __check(iy,ny,2,2);
  ...
  array2<T>& operator += (const array2<T>& A) {
    __checkSize();
    for(uintptr_t i=0; i < this->size; i++) this->v[i] += A(i);
    return *this;
  }
  array2<T>& operator -= (const array2<T>& A) {
    __checkSize();
  ...
  array2<T>& operator += (T a) {
    __checkSize();
    uintptr_t inc=ny+1;
    for(uintptr_t i=0; i < this->size; i += inc) this->v[i] += a;
    return *this;
  }
  array2<T>& operator -= (T a) {
    __checkSize();
  ...
  array2<T>& operator *= (T a) {
    __checkSize();
    for(uintptr_t i=0; i < this->size; i++) this->v[i] *= a;
    return *this;
  }
  ...
  void Identity() {
    ...
    __checkSize();
    uintptr_t inc=ny+1;
    for(uintptr_t i=0; i < this->size; i += inc) this->v[i]=(T) 1;
  }
};
...
class array3 : public array1<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0) {
    nx=nx0; ny=ny0; nz=nz0; nyz=ny*nz;
    this->size=nx*nyz;
  }
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0, T *v0) {
    Dimension(nx0,ny0,nz0);
    this->v=v0;
    this->clear(this->allocated);
  }
	
  void Allocate(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		uintptr_t align=0) {
    Dimension(nx0,ny0,nz0);
  ...
  array3(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
	 uintptr_t align=0) {
    Allocate(nx0,ny0,nz0,align);
  }
  array3(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0, T *v0) {
    Dimension(nx0,ny0,nz0,v0);
  }
	
  uintptr_t Nx() const {return nx;}
  uintptr_t Ny() const {return ny;}
  uintptr_t Nz() const {return nz;}

  array2<T> operator [] (intptr_t ix) const {
    __check(ix,nx,3,1);
    return array2<T>(ny,nz,this->v+ix*nyz);
  }
  T& operator () (intptr_t ix, intptr_t iy, intptr_t iz) const {
    __check(ix,nx,3,1);
    __check(iy,ny,3,2);
    __check(iz,nz,3,3);
...
class array4 : public array1<T> {
 ...
 public:
  ...
  void Allocate(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		uintptr_t nw0, uintptr_t align=0) {
    Dimension(nx0,ny0,nz0,nw0);
    __checkActivate(4,align);
  }
  
  array4() : nx(0), ny(0), nz(0), nw(0), nyz(0), nzw(0), nyzw(0) {}
  array4(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
	 uintptr_t nw0, uintptr_t align=0) {Allocate(nx0,ny0,nz0,nw0,align);}
  array4(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
	 uintptr_t nw0, T *v0) {
  ...
  uintptr_t Nx() const {return nx;}
  uintptr_t Ny() const {return ny;}
  uintptr_t Nz() const {return ny;}
  uintptr_t N4() const {return nw;}

  array3<T> operator [] (intptr_t ix) const {
    __check(ix,nx,3,1);
    return array3<T>(ny,nz,nw,this->v+ix*nyzw);
  }
  T& operator () (intptr_t ix, intptr_t iy, intptr_t iz, intptr_t iw) const {
    __check(ix,nx,4,1);
    __check(iy,ny,4,2);
    __check(iz,nz,4,3);
    __check(iw,nw,4,4);
...
class array5 : public array1<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		 uintptr_t nw0, uintptr_t nv0) {
    nx=nx0; ny=ny0; nz=nz0; nw=nw0; nv=nv0; nwv=nw*nv; nzwv=nz*nwv;
    nyzwv=ny*nzwv;
    this->size=nx*nyzwv;
  }
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		 uintptr_t nw0, uintptr_t nv0, T *v0) {
  ...
  void Allocate(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		uintptr_t nw0, uintptr_t nv0, uintptr_t align=0) {
    Dimension(nx0,ny0,nz0,nw0,nv0);
    __checkActivate(5,align);
  }
  
  array5() : nx(0), ny(0), nz(0), nw(0), nv(0), nwv(0), nzwv(0), nyzwv(0) {}
  array5(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
	 uintptr_t nw0, uintptr_t nv0, uintptr_t align=0) {
  ...
  uintptr_t Nz() const {return ny;}
  uintptr_t N4() const {return nw;}
  uintptr_t N5() const {return nv;}

  array4<T> operator [] (intptr_t ix) const {
    __check(ix,nx,4,1);
    return array4<T>(ny,nz,nw,nv,this->v+ix*nyzwv);
  }
  T& operator () (intptr_t ix, intptr_t iy, intptr_t iz, intptr_t iw, intptr_t iv) const {
    __check(ix,nx,5,1);
    __check(iy,ny,5,2);
    __check(iz,nz,5,3);
    __check(iw,nw,5,4);
  ...
  array5<T>& operator += (T a) {
    __checkSize();
    uintptr_t inc=nyzwv+nzwv+nwv+nv+1;
    for(uintptr_t i=0; i < this->size; i += inc) this->v[i] += a;
    return *this;
  }
  array5<T>& operator -= (T a) {
...
class Array1 : public array1<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, intptr_t ox0=0) {
    this->size=nx0;
    ox=ox0;
    Offsets();
  }
  void Dimension(uintptr_t nx0, T *v0, intptr_t ox0=0) {
    this->v=v0;
    Dimension(nx0,ox0);
    this->clear(this->allocated);
  }
  ...
  void Allocate(uintptr_t nx0, intptr_t ox0=0, uintptr_t align=0) {
    Dimension(nx0,ox0);
    __checkActivate(1,align);
    Offsets();
  }
	
  void Reallocate(uintptr_t nx0, intptr_t ox0=0, uintptr_t align=0) {
    this->Deallocate();
    Allocate(nx0,ox0,align);
  }

  Array1() : ox(0) {}
  Array1(uintptr_t nx0, intptr_t ox0=0, uintptr_t align=0) {
    Allocate(nx0,ox0,align);
  }
  Array1(uintptr_t nx0, T *v0, intptr_t ox0=0) {
    Dimension(nx0,v0,ox0);
  }
  Array1(T *v0) {
    Dimension(INTPTR_MAX,v0);
  }
  ...
  T& operator [] (intptr_t ix) const {__check(ix,this->size,ox,1,1); return voff[ix];}
  T& operator () (intptr_t i) const {__check(i,this->size,0,1,1); return this->v[i];}
  T* operator () () const {return voff;}
  operator T* () const {return voff;}
	
  Array1<T> operator + (intptr_t i) const {return Array1<T>(this->size-i,this->v+i,ox);}
...
class Array2 : public array2<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, intptr_t ox0=0, intptr_t oy0=0) {
    this->nx=nx0; this->ny=ny0;
    this->size=this->nx*this->ny;
    ox=ox0; oy=oy0;
    Offsets();
  }
  void Dimension(uintptr_t nx0, uintptr_t ny0, T *v0, intptr_t ox0=0,
		 intptr_t oy0=0) {
    this->v=v0;
  ...
  void Allocate(uintptr_t nx0, uintptr_t ny0, intptr_t ox0=0, intptr_t oy0=0,
		uintptr_t align=0) {
    Dimension(nx0,ny0,ox0,oy0);
    __checkActivate(2,align);
    Offsets();
  ...
  Array2() : ox(0), oy(0) {}
  Array2(uintptr_t nx0, uintptr_t ny0, intptr_t ox0=0, intptr_t oy0=0,
	 uintptr_t align=0) {
    Allocate(nx0,ny0,ox0,oy0,align);
  }
  Array2(uintptr_t nx0, uintptr_t ny0, T *v0, intptr_t ox0=0, intptr_t oy0=0) {
...
class Array3 : public array3<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		 intptr_t ox0=0, intptr_t oy0=0, intptr_t oz0=0) {
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		 T *v0, intptr_t ox0=0, intptr_t oy0=0, intptr_t oz0=0) {
    this->v=v0;
  ...
  void Allocate(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		intptr_t ox0=0, intptr_t oy0=0, intptr_t oz0=0, uintptr_t align=0) {
    Dimension(nx0,ny0,nz0,ox0,oy0,oz0);
    __checkActivate(3,align);
    Offsets();
  }
...
class Array4 : public array4<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		 uintptr_t nw0, 
		 intptr_t ox0=0, intptr_t oy0=0, intptr_t oz0=0, intptr_t ow0=0) {
  ...
  void Allocate(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		uintptr_t nw0,
		intptr_t ox0=0, intptr_t oy0=0, intptr_t oz0=0, intptr_t ow0=0, uintptr_t align=0) {
    Dimension(nx0,ny0,nz0,nw0,ox0,oy0,oz0,ow0);
    __checkActivate(4,align); 
    Offsets();
  }
...
class Array5 : public array5<T> {
 ...
 public:
  ...
  void Dimension(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
		 uintptr_t nw0,  uintptr_t nv0,
		 intptr_t ox0=0, intptr_t oy0=0, intptr_t oz0=0, intptr_t ow0=0, intptr_t ov0=0) {
  ...
  Array5(uintptr_t nx0, uintptr_t ny0, uintptr_t nz0,
	 uintptr_t nw0, uintptr_t nv0, intptr_t ox0=0, intptr_t oy0=0,
	 intptr_t oz0=0, intptr_t ow0=0, intptr_t ov0=0, uintptr_t align=0) {
...
template<class T>
inline void Dimension(T *&, uintptr_t)
{
}

template<class T>
inline void Dimension(array1<T> &A, uintptr_t n)
{
  A.Dimension(n);
}

template<class T>
inline void Dimension(T *&A, uintptr_t, T *v)
{
  A=v;
}

template<class T>
inline void Dimension(array1<T>& A, uintptr_t n, T *v)
{
  A.Dimension(n,v);
}
...
template<class T>
inline void Dimension(array1<T>& A, uintptr_t n, const array1<T>& B)
{
  A.Dimension(n,B);
}

template<class T>
inline void Dimension(Array1<T>& A, uintptr_t n, const array1<T>& B, intptr_t o)
{
  A.Dimension(n,B,o);
}

template<class T>
inline void Dimension(Array1<T>& A, uintptr_t n, T *v, intptr_t o)
{
  A.Dimension(n,v,o);
}

template<class T>
inline void Dimension(T *&A, uintptr_t, T *v, intptr_t o)
{
  A=v-o;
}

template<class T>
inline void Allocate(T *&A, uintptr_t n, uintptr_t align=0)
{
...
template<class T>
inline void Allocate(array1<T>& A, uintptr_t n, uintptr_t align=0)
{  
  A.Allocate(n,align);
}
...
template<class T>
inline void Allocate(T *&A, uintptr_t n, intptr_t o, uintptr_t align=0)
{
  Allocate(A,n,align);
  A -= o;
}
...
template<class T>
inline void Allocate(Array1<T>& A, uintptr_t n, intptr_t o, uintptr_t align=0)
{  
...
template<class T>
inline void Deallocate(T *A, intptr_t o)
{
  if(A) delete [] (A+o);
}
...
template<class T>
inline void Reallocate(T *&A, uintptr_t n, uintptr_t align=0)
{  
  if(A) delete [] A;
  Allocate(A,n,align);
}
...
template<class T>
inline void Reallocate(array1<T>& A, uintptr_t n)
{  
  A.Reallocate(n);
}
...
template<class T>
inline void Reallocate(T *&A, uintptr_t n, intptr_t o, uintptr_t align=0)
{  
  if(A) delete [] A;
  Allocate(A,n,align);
  A -= o;
...
template<class T>
inline void Reallocate(Array1<T>& A, uintptr_t n, intptr_t o, uintptr_t align=0)
{  
  A.Reallocate(n,o,align);
}

template<class T>
inline void CheckReallocate(T& A, uintptr_t n, uintptr_t& old,
			    uintptr_t align=0)
{
...
template<class T>
inline void CheckReallocate(T& A, uintptr_t n, intptr_t o, uintptr_t& old,
			    uintptr_t align=0)
{
